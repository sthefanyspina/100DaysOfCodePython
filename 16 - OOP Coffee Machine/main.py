...
##Printing report of all resources, checking if resources are sufficient, making coffe
...
on = True
while on:
    choice = input(f"What would like? Choose between espresso, latte or capuccino: ")
    if choice == 'off':
        ...
        if coffee_maker.is_resource_sufficient(espresso) and money_machine.make_payment(espresso.cost):
            coffee_maker.make_coffe(coffee_choice)
