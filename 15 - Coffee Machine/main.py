...
def report():
    ...
    print(f"Money: {resources['value']} ml")

def check_resources(coffe_choice):
    enough = True
    if MENU[coffe_choice]['ingredients']['water'] > resources['water']:
        print(f"There isn't enough water for a {coffe_choice}")
        enough = False
    if MENU[coffe_choice]['ingredients']['coffee'] > resources['coffee']:
        print(f"There isn't enough coffee for a {coffe_choice}")
        enough = False
    if coffe_choice != 'espresso':
        if MENU[coffe_choice]['ingredients']['milk'] > resources['milk']:
            print(f"There isn't enough milk for a {coffe_choice}")
            enough = False
    return enough
    ...
    while on:
        ...
        if choice == 'report':
            ...
            if process_coins(quarters, dimes, nickles, pennies) == MENU[choice]['cost']:
                ...
                print(f"Here us your {choice}. Have a good day!")
                ...
                print(f"Here us ${change} in change")
                print(f"Here us your {choice}. Have a good day!")
            else:
